...
//	if (!isDistSensorOK) isCommandDone=true;
	if ((distance - distanceTraveled) > 1000 or (distance - distanceTraveledR) > 1000) {
		leftMotorOutput = -0.8;
	...
	if (distanceTraveled>distanceTraveledR) {
		rightMotorOutput -= 0.1;
	}
	else if (distanceTraveled<distanceTraveledR) {
		leftMotorOutput -= 0.1;
	}
	...
	if(distanceTraveled>distance or distanceTraveledR>distance)isCommandDone=true;
