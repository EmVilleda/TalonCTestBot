...
void Robot::RobotInit() {
	...
	imaqError = IMAQdxOpenCamera("cam1", IMAQdxCameraControlModeController, &camera);
	if(imaqError != IMAQdxErrorSuccess) {
		DriverStation::ReportError("IMAQdxOpenCamera 2 error: " + std::to_string((long)imaqError) + "\n");
	}
	imaqError = IMAQdxConfigureGrab(camera);
	if(imaqError != IMAQdxErrorSuccess) {
...
//	driveCommand->Start();
	...
	IMAQdxSession& capture_cam = camera;
	IMAQdxGrab(capture_cam, frame, true, NULL);
	if(imaqError != IMAQdxErrorSuccess) {
		DriverStation::ReportError("IMAQdxGrab error camera1: " + std::to_string((long)imaqError) + "\n");
	} else {
